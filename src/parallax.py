...
class ParallaxLayer:
    ...
    def draw(self, surface, camera_offset_x, draw_y=0):
        ...
        for i in range(start_tile, start_tile + tiles_needed + 1):
            ...
            surface.blit(self.image, (draw_pos_x, draw_y))

class ParallaxObject:
    def __init__(self, image, speed_ratio, y_pos, x_offset=0):
        self.image = image
        self.speed_ratio = speed_ratio
        self.y_pos = y_pos
        self.x_offset = x_offset  

    def draw(self, surface, camera_offset_x, camera_offset_y=-23):
        img_width = self.image.get_width()
        surface_width = surface.get_width()

        scroll = camera_offset_x * self.speed_ratio

        draw_x = surface_width - img_width - 200 - scroll + self.x_offset
        surface.blit(self.image, (draw_x, self.y_pos + camera_offset_y))
